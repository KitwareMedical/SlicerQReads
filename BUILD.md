Build and Package SlicerQReads
==============================
...
* Setting up your git account:
    ...
    * If not already done, email `FirstName LastName <firstname.lastname@KitwareMedical.com>` to be granted access to
    the [KitwareMedical/SlicerQReads](https://github.com/KitwareMedical/SlicerQReads) repository.
...
mkdir W
cd /c/W
git clone https://github.com/KitwareMedical/SlicerQReads.git SlicerQReads
```
...
1. Start [CMake GUI](https://cmake.org/runningcmake/), select source directory `C:\W\SlicerQReads` and set build directory to `C:\W\SlicerQReads-rel`.
2. Add an entry `Qt5_DIR` pointing to `C:/Qt/${QT_VERSION}/${COMPILER}/lib/cmake/Qt5`.
2. Generate the project.
3. Open `C:\W\SlicerQReads-rel\SlicerQReads.sln`, select `Release` and build the project.

<b>Option 2: Command Line</b>

1. Start the [Command Line Prompt](http://windows.microsoft.com/en-us/windows/command-prompt-faq)
2. Configure and build the project in `C:\W\SlicerQReads-rel` by typing the following commands:
...
1. In the `C:\W\SlicerQReads-rel\Slicer-build` directory, open `Slicer.sln` and build the `PACKAGE` target
